						...
						<div class="single-comment row border-bottom py-1 my-1">
							<div class="col-sm-2 text-center">
								<img src="<?=get_image($com->user_row->image ?? '')?>" class="img-thumbnail rounded-circle" style="width:100%;max-width:100px">
									<?php if($com->user_row->role == 'admin'):?>
										<div class="badge bg-warning text-dark bg-gradient text-wrap mb-2"><?=ucwords($com->user_row->username) ?? 'Admin'?></div>
									<?php else:?>
										<div class="badge bg-secondary bg-gardient text-warp mb-2"><?=ucwords($com->user_row->username) ?? 'Unknown'?></div>			
									<?php endif;?>	
							</div>
							<div class="col-sm-10">
